...
import type { BiquadFilterFlowNode, BiquadFilterType } from '../../../store/dawTypes';
...
export const BiquadFilterNode = memo(function BiquadFilterNode({ id, data, selected }: NodeProps<BiquadFilterFlowNode>) {
	...
	return (
		<Box sx={{ borderRadius: 1, backgroundImage: METAL_BG, width: 2.5 * GRID_UNIT, position: 'relative', boxShadow: selected ? `0px 4px 15px ${color}4d` : '0px 4px 15px rgba(0,0,0,0.30)' }}>
			<NodeHeader id={id} label='BiquadFilter' selected={selected} accentColor={color} filledHeader />
