...
export const FeedbackDelayNode = memo(function FeedbackDelayNode({ id, data, selected }: NodeProps<FeedbackDelayFlowNode>) {
	...
	return (
		<Box sx={{ border: '1px solid', borderColor: color, borderRadius: 1, backgroundImage: METAL_BG, width: 2 * GRID_UNIT, position: 'relative' }}>
			<NodeHeader id={id} label='FeedbackDelay' selected={selected} accentColor={color} />

			<Box sx={{ px: 1, pt: 2, pb: 0.75, display: 'flex', flexDirection: 'column', gap: 0.75 }} className='nodrag nowheel'>
				<HwSliderField label='time'     value={delayTime} min={0} max={1} step={0.01} color={color} onChange={v => { setDelay(v);    setFeedbackDelayTime(id, v);     }} format={v => v.toFixed(2)} unit='s' allowValueEdit allowBoundsEdit />
				<HwSliderField label='feedback' value={feedback}  min={0} max={1} step={0.01} color={color} onChange={v => { setFeedback(v); setFeedbackDelayFeedback(id, v); }} format={v => v.toFixed(2)}          allowValueEdit allowBoundsEdit />
				<HwSliderField label='wet'      value={wet}       min={0} max={1} step={0.01} color={color} onChange={v => { setWet(v);      setFeedbackDelayWet(id, v);      }} format={v => v.toFixed(2)}          allowValueEdit />
			</Box>
