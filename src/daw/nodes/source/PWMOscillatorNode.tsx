...
export const PWMOscillatorNode = memo(function PWMOscillatorNode({ id, data, selected }: NodeProps<PWMOscillatorFlowNode>) {
	...
	return (
		<Box sx={{ borderRadius: 1, backgroundImage: METAL_BG, width: 2 * GRID_UNIT, position: 'relative', pb: 2, boxShadow: selected ? `0px 4px 15px ${color}4d` : '0px 4px 15px rgba(0,0,0,0.30)' }}>
			<NodeHeader id={id} label='PWM Osc' selected={selected} accentColor={color} filledHeader />
