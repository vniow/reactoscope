...
export function SixChannelNoiseGeneratorNode({
	...
	// Refs for transformable objects
	// Removed refs since shape components do not support forwardRef
			...
			<>
				<div className='flex flex-col gap-3'>
					...
					<View
						...
						aria-label='3D noise generator window'
					>
						<ambientLight intensity={1} />
						<group>
							{/* If these components do not render, try replacing with fallback meshes below */}
							<TriangleShape />
							<SquareShape />
							<PolygonShape sides={polygonSides} />
							{/*
							   <mesh position={[-1,0,0]}>
				...
				{/* Handles: render outside the main controls div, but inside the fragment */}
				{[0, 1, 2, 3, 4, 5].map((idx) => (
					<NodeHandle
						key={idx}
						id={`output-${idx + 1}`}
						...
						connectionStatus={
							edges.some(
								(edge) =>
									edge.source === nodeId &&
									edge.sourceHandle === `output-${idx + 1}`
							)
								? 'connected'
								: 'default'
						}
	...
	// Removed duplicate and stray code after main return
}
