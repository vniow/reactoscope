...
export function SimpleXYScope({ id }: NodeProps<CustomNode>) {
	const { classes } = useNodeTheme('oscilloscope');
	...
	const getStatusColor = () => {
		if (xConnected && yConnected) return 'indicator-active';
		if (xConnected || yConnected) return 'indicator-warning';
		return 'indicator-error';
	};

	return (
		<div className={classes.container}>
			<div className={classes.header}>XY Oscilloscope</div>

			{/* Canvas for XY visualization */}
			<div className='bg-black border oscilloscope-node-border rounded mb-3'>
				<canvas
					ref={canvasRef}
					width={300}
					height={300}
			...
			<div className='flex items-center justify-center text-xs mb-2'>
				<div className={cn('font-mono', getStatusColor())}>
					Status: {connectionStatus()}
				</div>
			</div>

			{/* Input labels */}
			<div className='flex justify-between text-xs oscilloscope-node-accent font-mono mb-2'>
				<div className={xConnected ? 'indicator-active' : 'muted-text'}>
					X: {xConnected ? 'CONNECTED' : 'NO SIGNAL'}
				</div>
				<div className={yConnected ? 'indicator-active' : 'muted-text'}>
					Y: {yConnected ? 'CONNECTED' : 'NO SIGNAL'}
				</div>
			</div>

			<div className={classes.subtitle}>📊 XY MODE DISPLAY</div>
			...
			{/* Handle labels */}
			<div className='absolute left-4 top-[40%] transform -translate-y-1/2 text-xs indicator-error font-mono font-bold'>
				X
			</div>
			<div className='absolute left-4 top-[60%] transform -translate-y-1/2 text-xs indicator-active font-mono font-bold'>
				Y
			</div>
		</div>
	);
