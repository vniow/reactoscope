...
const DebugTooltip = ({
	...
	const tooltipContent = [
		...
		handle.offsetX !== undefined && `OffsetX: ${handle.offsetX}`,
		handle.offsetY !== undefined && `OffsetY: ${handle.offsetY}`,
		handle.offsetMode && `Mode: ${handle.offsetMode}`,
...
/**
 * Convert grid coordinates to ReactFlow position and offset with custom offset support
 */
function getPositionFromGridCoords(
	...
	customOffsetY: number = 0,
	offsetMode: 'pixels' | 'grid-units' = 'pixels'
): { position: Position; offset: { x: number; y: number } } {
	// Calculate custom offsets in pixels
	const pixelOffsetX = calculateHandleOffset(customOffsetX, offsetMode);
	const pixelOffsetY = calculateHandleOffset(customOffsetY, offsetMode);
	...
	if (gridY === 0) {
		...
		return {
			...
			offset: {
				x: gridX * GRID_UNIT + GRID_UNIT / 2 + pixelOffsetX,
				y: pixelOffsetY,
			},
		};
		...
		return {
			...
			offset: {
				x: pixelOffsetX,
				y: gridY * GRID_UNIT + GRID_UNIT / 2 + pixelOffsetY,
			},
		};
...
export function GridNodeHandle({
	...
	}, [id, gridX, gridY, offsetX, offsetY, offsetMode, offset, position]);
