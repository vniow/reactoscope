 ...
 */
export function useVariantColors(variant: ComponentVariant): VariantColorTokens {
 ...
 */
export function useVariantButtonStyles(variant: ComponentVariant): React.CSSProperties {
 ...
 */
export function useMultiVariantColors(variants: ComponentVariant[]): Record<ComponentVariant, VariantColorTokens> {
	return useMemo(() => {
...
export function useVariantStyleApplication(
	...
	return useMemo(() => ({
		...
		style: tokens.reduce((acc, token) => {
			(acc as Record<string, string>)[`--current-${token}`] = colorTokens[token];
			return acc;
