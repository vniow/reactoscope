 ...
 */
export class XYRGBInterpolatorNode {
	readonly name: string = 'XYRGBInterpolatorNode';
	readonly outputs: {
		x: Tone.Gain;
		y: Tone.Gain;
		r: Tone.Gain;
	...
	/**
	 * Create a new XYRGBInterpolatorNode
	 */
	...
	private async _initializeWorklet(): Promise<void> {
		try {
			...
			const workletBlob = new Blob([xyrgbInterpolatorWorklet], {
			...
			console.error('❌ Failed to initialize XYRGBInterpolatorNode:', error);
			throw error;
		}
	}
	...
	start(): this {
		if (this._workletNode && this._isReady && !this._isPlaying) {
			...
			if (this._debug) {
				console.log('▶️ XYRGBInterpolatorNode started');
			}
		}
		return this;
	}
	...
	stop(): this {
		if (this._workletNode && this._isReady && this._isPlaying) {
			...
			if (this._debug) {
				console.log('⏹️ XYRGBInterpolatorNode stopped');
			}
		}
		return this;
	}
	 ...
	 */
	setInterpolationMode(mode: 'linear' | 'cubic' | 'circular'): this {
	...
	dispose(): this {
		...
		if (this._debug) {
			console.log('🧹 XYRGBInterpolatorNode disposed');
		}
